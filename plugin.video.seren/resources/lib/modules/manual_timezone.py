...
def notify_timezone_not_detected():
    confirm = xbmcgui.Dialog().yesno(g.get_language_string(30591), g.get_language_string(30592))
    if confirm:
        choose_timezone()
...
def choose_timezone():
    ...
    tz_index = xbmcgui.Dialog().select(g.get_language_string(30590), time_zones)
    if not tz_index == -1:
