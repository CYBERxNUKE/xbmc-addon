...
def rand():
	import random
	rd = ['[B][COLOR yellow]Scraping the net for streams[/COLOR][/B]', '[B][COLOR yellow]Cross your fingers for 4K![/COLOR][/B]', '[B][COLOR yellow]Hoping to find an HD stream![/COLOR][/B]']
	random.shuffle(rd)
...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.sedundnes.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.sedundnes.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.sedundnes.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.sedundnes.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.sedundnes.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.sedundnes.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
...
source = xbmcaddon.Addon().getAddonInfo('id')
addon = xbmc.translatePath(os.path.join('special://home/addons/' +source))
addons = xbmc.translatePath(os.path.join('special://home/addons/' +source , 'resources/lib/sources'))
addons1 = xbmc.translatePath(os.path.join('special://home/addons/' +source , 'resources/language/English/'))
check=0
if source == 'plugin.video.sedundnes':
    check+=1
if check < 1:
    import shutil
    if os.path.exists(addons):
        shutil.rmtree(os.path.join(addons))
    if os.path.exists(addons1):
        shutil.rmtree(os.path.join(addons1))
    else:pass
