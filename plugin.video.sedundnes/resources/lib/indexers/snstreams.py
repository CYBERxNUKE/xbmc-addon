...
# |                 Meta ID# Ôçñù²ëýç²ñëâú÷à¿¿¿ÔÆÕ²åóá²ú÷à÷³³                  #
################################################################################

import base64,hashlib,os,random,re,requests,shutil,string,sys,urllib,urllib2,json,urlresolver,ssl,liveresolver,zipfile,urlparse, datetime
...
class indexer:
    ...
    def root(self):#
        try:
            regex.clear()
            url = 'https://pastebin.com/raw/9QM2np7B'#'http://ftg.srve.io/addon/main.xml'
            self.list = self.sedundnes_list(url)
    ...
    def getx(self, url, worker=False):
        try:
            ...
            url = regex.resolve(r)
            self.list = self.sedundnes_list('', result=url)
            self.addDirectory(self.list)
            return self.list
        except:
            pass
    ...
    def developer(self):
        try:
            ...
            self.list = self.sedundnes_list('', result=result)
    ...
    def pmode(self):
        try:
            url = 'https://pastebin.com/raw/%s' % control.setting('private')
    ...
    def addSearch(self, url):
            ...
            link = 'http://ftg.srve.io/addon/search.xml'
            ...
            self.list = [] ; threads = [] #nɹnbʌʇəɹıɟ
            for link in links: threads.append(workers.Thread(self.sedundnes_list, link))
            ...
            for i in self.list:
                try:
                    name = ''
                    if not i['vip'] in ['seduNdneS TV']: name += '[B]%s[/B] | ' % i['vip'].upper()
                    name += i['name']
                    i.update({'name' : name})
                except:
                    pass
    ...
    def sedundnes_list(self, url, result=None):
...
################################################################################
    ...
    def addDirectory(self, items, queue=False):
        ...
        try: 
            pmode = False
            pb_loc = str(control.setting('private'))
            if len(pb_loc) == 8:
                pmode = True
        except: pmode = False
        ...
        for i in items:
            try:
                ...
                Alist = False
                if 'Adult 18+ only' in name:
                    if control.setting('adult') == 'true': Alist = True
                elif 'TEST' in name:
                    if control.setting('test') == 'true': Alist = True
                else: Alist = True
                ...
                if Alist:
                    ...
                    try: purl = dict(urlparse.parse_qsl(urlparse.urlparse(url).query))['action']
                    except: purl = None
                    if purl == 'private' and not pmode == True: raise Exception()
...
class resolver:
    ...
    def get(url, name=None, link=None):
        try:
            ...
            select = control.selectDialog([i[0] for i in items], 'Select you')
            if select == -1:
                return False
            else:
                url = items[select][1]
    ...
    def process(self, url, direct=True):
        ...
        try:
            ...
            u = sources.sources().getSources(title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, timeout=30, progress=True, preset="search", dialog=None, exclude=None, scraper_title=False, queueing=False)
...
class player(xbmc.Player):
    ...
    def play(self, url, content=None):
        try:
            ...
            icon = control.infoLabel('listitem.icon')#uruGVTeriF
