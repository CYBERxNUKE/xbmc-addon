...
import freshstart
import configwizard, configwizard1, installer
...
def main_menu():
    ...
    try:
        if not os.path.exists(hubpath):
            installer.HUBINSTALL('repository.cyberxnuke','https://github.com/CYBERxNUKE/xbmc-addon/raw/master/repository.cyberxnuke/repository.cyberxnuke-1.2.zip','','addon','none')
    ...
    kodi.addItem("Update",'','call_wizard1',artwork+'config_wizard.png',description="Automatically configure Kodi with the new updates!")
    ...
    kodi.addItem("Refresh Kodi",'','call_rejuv',artwork+'rejuvinate.png',description="Wipe and reconfigure Kodi with the latest Computertechs Installer setup!")
...
def fullspeedtest():
    ...
    try:
        ...
        for url, iconimage, name in reversed(match):
            iconimage = artwork + 'speed_test.png'
...
######MAIN TOOL
    ...
    if _is_debugging() == True:
        choice = xbmcgui.Dialog().yesno('COMPUTERTECHS INSTALLER', 'Please confirm that you wish to disable log debugging mode immediately')
        if choice == 1:
            xbmc.executebuiltin("ToggleDebug")
        else: quit()
    else:
        choice = xbmcgui.Dialog().yesno('COMPUTERTECHS INSTALLER', 'Please confirm that you wish to enable log debugging mode immediately')
        if choice == 1:
            xbmc.executebuiltin("ToggleDebug")
        else: quit()
    ...
    choice = xbmcgui.Dialog().yesno('COMPUTERTECHS INSTALLER', 'Please confirm that you wish to reload the skin cache immediately.')
    if choice == 1:
        xbmc.executebuiltin("ReloadSkin()")
    else: quit()
    
elif mode== 'updateaddons':
    choice = xbmcgui.Dialog().yesno('COMPUTERTECHS INSTALLER', 'Please confirm that you wish to force update all addons and repositories immediately.')
    if choice == 1:
    ...
    choice = xbmcgui.Dialog().yesno('COMPUTERTECHS INSTALLER', 'Would you like to hide or unhide your passwords?',yeslabel='Hide',nolabel='Unhide')
...
#######UPDATE#########################
elif mode=='call_wizard1':
        configwizard1.HELPCATEGORIES()

elif mode=="wizardstatus1":
        print""+url
        items=configwizard1.WIZARDSTATUS(url)
elif mode=='helpwizard1':
        configwizard1.HELPWIZARD(name,url,description,filetype)
