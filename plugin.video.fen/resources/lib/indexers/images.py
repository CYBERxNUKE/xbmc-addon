		...
		def builder():
			for item in image_info['results']:
				if item['profile_path']:
					actor_poster = 'http://image.tmdb.org/t/p/w185%s' % item['profile_path']
					actor_image = 'http://image.tmdb.org/t/p/h632%s' % item['profile_path']
				else:
					actor_poster = os.path.join(icon_directory, 'genre_family.png')
					actor_image = os.path.join(icon_directory, 'genre_family.png')
				url_params = {'mode': 'people_search.person_data_dialog', 'actor_name': item['name'], 'actor_id': item['id'], 'actor_image': actor_image}
