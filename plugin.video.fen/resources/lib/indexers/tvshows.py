...
watched_str, unwatched_str, traktmanager_str, favmanager_str, extras_str, options_str, random_str = ls(32642), ls(32643), ls(32198), ls(32197), ls(32645), ls(32646), ls(32611)
exit_str, nextpage_str, switchjump_str, browse_str, jumpto_str, genre_str = ls(32650), ls(32799), ls(32784), ls(32652), ls(32964), ls(32847)
...
class TVShows:
	...
	def fetch_list(self):
		try:
			...
			if self.action in personal:
				if self.action == 'favourites_tvshows': var_module, import_function = 'modules.favourites', 'retrieve_favourites'
				elif self.action == 'in_progress_tvshows': var_module, import_function = 'modules.watched_status', 'get_in_progress_tvshows'
			...
			if self.action in tmdb_main:
				...
				data = function(page_no)
				for item in data: list_append(get_trakt_tvshow_id(item['show']['ids']))
				self.new_page = {'new_page': string((data['page'] if 'tmdb' in self.action else page_no) + 1)}
				...
				for item in function(self.params['period'], self.params['duration'], page_no): list_append((get_trakt_tvshow_id(item['show']['ids'])))
				...
				for item in data: list_append(get_trakt_tvshow_id(item['ids']))
				if total_pages > page_no:
					self.new_page = {'new_page': string(page_no + 1), 'imdb_id': imdb_id, 'imdb_id': params_get('imdb_id')}
				...
				for item in function(self.params['certification'], page_no): list_append((get_trakt_tvshow_id(item['show']['ids'])))
				self.new_page = {'new_page': string(page_no + 1), 'certification': self.params['certification']}
			elif self.action == 'trakt_recommendations':
				for item in function('shows'): list_append(get_trakt_tvshow_id(item['ids']))
			elif self.action == 'tmdb_tv_search':
				query = self.params['query']
				data = function(query, page_no)
				total_pages = data['total_pages']
				if total_pages > page_no:
					self.new_page = {'new_page': string(page_no + 1), 'query': query}
				self.list = [i['id'] for i in data['results']]
	...
	def build_tvshow_content(self, item_position, _id):
		try:
			...
			extras_params = build_url({'mode': 'extras_menu_choice', 'tmdb_id': tmdb_id, 'db_type': 'tvshow', 'is_widget': self.is_widget})
			random_params = build_url({'mode': 'play_fetch_random', 'tmdb_id': tmdb_id})
			trakt_manager_params = build_url({'mode': 'trakt_manager_choice', 'tmdb_id': tmdb_id, 'imdb_id': imdb_id, 'tvdb_id': tvdb_id, 'db_type': 'tvshow'})
			fav_manager_params = build_url({'mode': 'favorites_choice', 'db_type': 'tvshow', 'tmdb_id': tmdb_id, 'title': title})
			...
			listitem.setArt({'poster': poster, 'fanart': fanart, 'icon': poster, 'banner': meta_get('banner'), 'clearart': meta_get('clearart'),
							'clearlogo': clearlogo, 'landscape': meta_get('landscape'), 'tvshow.clearart': meta_get('clearart'),'tvshow.clearlogo': clearlogo,
							'tvshow.landscape': meta_get('landscape'), 'tvshow.banner': meta_get('banner')})
			...
			set_property('fen_db_type', 'tvshow')
	...
	def set_meta(self, _id):
		...
		if not meta: return
		playcount, overlay, total_watched, total_unwatched = get_watched_status_tvshow(self.watched_info, string(meta['tmdb_id']), meta.get('total_aired_eps'))
