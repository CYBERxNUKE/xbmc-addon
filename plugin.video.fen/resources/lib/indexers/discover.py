...
class Discover:
	...
	def movie(self):
		...
		if not 'recommended' in names: self._add_dir({'mode': 'discover.similar_recommended', 'db_type': 'movie', 'key': 'similar', 'list_name': '[B]%s %s:[/B]  [I]%s[/I]' \
																												% (ls(32451), ls(32592), names.get('similar', ''))})
		if not 'similar' in names: self._add_dir({'mode': 'discover.similar_recommended', 'db_type': 'movie', 'key': 'recommended', 'list_name': '[B]%s %s:[/B]  [I]%s[/I]' \
		...
		if not any(i in names for i in ['similar', 'recommended']):
			...
			self._add_dir({'mode': 'discover.include_keywords', 'db_type': 'movie', 'list_name': self.base_str % (self.include_base_str \
																																% ls(32657), names.get('include_keywords', ''))})
			self._add_dir({'mode': 'discover.exclude_keywords', 'db_type': 'movie', 'list_name': self.base_str % (self.exclude_base_str \
	...
	def tvshow(self):
		...
		if not 'recommended' in names: self._add_dir({'mode': 'discover.similar_recommended', 'db_type': 'tvshow', 'key': 'similar', 'list_name': '[B]%s %s:[/B]  [I]%s[/I]' \
																														% (ls(32451), ls(32592), names.get('similar', ''))})
		if not 'similar' in names: self._add_dir({'mode': 'discover.similar_recommended', 'db_type': 'tvshow', 'key': 'recommended', 'list_name': '[B]%s %s:[/B]  [I]%s[/I]' \
																														% (ls(32451), ls(32593), names.get('recommended', ''))})
		if not any(i in names for i in ['similar', 'recommended']):
			...
			self._add_dir({'mode': 'discover.include_keywords', 'db_type': 'tvshow', 'list_name': self.base_str \
				                                                                         % (self.include_base_str % ls(32657), names.get('include_keywords', ''))})
			self._add_dir({'mode': 'discover.exclude_keywords', 'db_type': 'tvshow', 'list_name': self.base_str \
	...
	def include_genres(self):
		key = 'include_genres'
		if self._action(key) in ('clear', None): return
		if self.discover_params['db_type'] == 'movie': genres = meta_lists.movie_genres
	...
	def _add_defaults(self):
		...
		self._add_dir({'mode': 'discover.export', 'db_type': self.db_type, 'list_name': ls(32667) % name}, icon=kodi_utils.translate_path(icon_directory % 'nextpage.png'))
	...
	def _build_url(self, query):
		return 'plugin://plugin.video.fen/?' + urlencode(to_utf8(query))
	...
	def _listitem_position(self, key):
		if self.db_type == 'movie' and key in ('rating', 'rating_votes', 'sort_by'): key = '%s_movie' % key
