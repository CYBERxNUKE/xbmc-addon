...
class Extras(BaseDialog):
		...
		if controlID in button_ids:
			if controlID == playbrowse_id:
				...
				if self.media_type == 'movie':
					...
					function = dialogs.random_choice({'meta': self.meta, 'poster': self.poster, 'return_choice': 'true', 'window_xml': 'media_select.xml'})
	...
	def make_collection(self):
		...
		try:
			data = movieset_meta(coll_id, settings.metadata_user_info())
