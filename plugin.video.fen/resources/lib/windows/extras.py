...
mpaa_icon = 'special://home/addons/script.tikiart/resources/media/mpaa/%s.png'
...
play_id, trailer_id, keywords_id, images_id, extrainfo_id, genre_id, directorrandom_id, trakt_id, plot_id = button_ids
...
default_text_color, default_highlight_color = 'FF302F2F', 'FFB7B4BB'
...
class Extras(BaseDialog):
	...
	def onInit(self):
		...
		Thread(target=self.make_artwork, args=('posters',)).start()
		Thread(target=self.make_artwork, args=('backdrops',)).start()
		if self.db_type == 'movie': Thread(target=self.make_collection).start()
	...
	def onClick(self, controlID):
		...
		if controlID in button_ids:
			if controlID == play_id:
				if self.db_type == 'movie':
					url_params = {'mode': 'play_media', 'vid_type': 'movie', 'query': self.rootname, 'tmdb_id': self.tmdb_id}
				...
				chosen = dialogs.trailer_choice(self.db_type, self.poster, self.tmdb_id, self.meta['trailer'], self.meta['all_trailers'])
				...
				close_all_dialog()
				self.selected = (self.folder_runner % self.build_url(json.loads(keyword_params)))
				self.close()
				...
				close_all_dialog()
				self.selected = (self.folder_runner % self.build_url(json.loads(genre_params)))
				self.close()
				...
				params = {'tmdb_id': self.tmdb_id, 'imdb_id': self.imdb_id, 'tvdb_id': self.meta['tvdb_id'], 'db_type': self.db_type}
	...
	def onAction(self, action):
		...
		if action in self.context_actions:
			...
			if focus_id == play_id and self.db_type == 'movie':
				...
				params = {'action': 'image', 'name': '%s %s' % (self.rootname, chosen_listitem.getProperty('tikiskins.extras.name')),
						...
						'db_type': 'image', 'image': translate_path('special://home/addons/plugin.video.fen/icon.png')}
	...
	def get_next_episode(self):
		...
		try: info = [i for i in ep_list if i['tmdb_id'] == str(self.tmdb_id)][0]
		except: return ''
	...
	def set_poster(self):
		if self.current_poster:
			...
			while not self.check_poster_cached(self.poster): self.sleep(50)
	...
	def set_hex_values(self):
		if not self.enable_highlight_colors: return self.set_home_property('extras.maincolor_hex', default_highlight_color)
		...
		self.set_home_property('extras.maincolor_hex', maincolor_hex)
...
class ShowSelectMedia(BaseDialog):
	def __init__(self, *args, **kwargs):
		BaseDialog.__init__(self, args)
		self.items = kwargs['items']
		self.poster = kwargs['poster']
		self.context_active_action = kwargs.get('context_active_action', '')
		self.window_id = 2000
		self.selected = None
		self.set_properties()
		self.make_menu()
	...
	def onAction(self, action):
		if action in self.selection_actions:
			...
			self.selected = chosen_listitem.getProperty('tikiskins.select_media.function')
			return self.close()
		elif action in self.context_actions and self.context_active_action:
			if self.context_active_action == 'keywords':
				chosen_listitem = self.get_listitem(self.window_id)
				function = chosen_listitem.getProperty('tikiskins.select_media.function')
				list_id = json.loads(function)['list_id']
				cm_choice = self.open_window(['windows.extras', 'SelectMediaContextMenu'], 'contextmenu.xml',
											item=function, context_active_action=self.context_active_action, list_id=list_id)
				if not cm_choice: return
	...
	def make_menu(self):
		def builder():
			for item in self.items:
				listitem = self.make_listitem()
				listitem.setProperty('tikiskins.select_media.line', item['line'])
				listitem.setProperty('tikiskins.select_media.function', item['function'])
				yield listitem
		self.item_list = list(builder())

	def set_properties(self):
		self.setProperty('tikiskins.select_media.poster', self.poster)

class SelectMediaContextMenu(BaseDialog):
	def __init__(self, *args, **kwargs):
		BaseDialog.__init__(self, args)
		self.window_id = 2020
		self.kwargs = kwargs
	...
	def make_menu(self):
		if self.context_active_action == 'keywords':
			keyword = self.kwargs['list_id'].upper()
			add_list_params = {'mode': 'navigator.adjust_main_lists', 'method': 'add_external', 'list_name': '%s (IMDb)' % keyword, 'menu_item': self.item}
			add_folder_params = {'mode': 'navigator.adjust_shortcut_folder_lists', 'method': 'add_external', 'name': '%s (IMDb)' % keyword, 'menu_item': self.item}
			self.item_list.append(self.make_contextmenu_item(ls(32730), 'RunPlugin(%s)', add_list_params))
			self.item_list.append(self.make_contextmenu_item(ls(32731), 'RunPlugin(%s)', add_folder_params))
