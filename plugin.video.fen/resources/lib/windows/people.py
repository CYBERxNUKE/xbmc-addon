...
more_from_movies_id = 2050
more_from_tvshows_id = 2051
imdb_videos_id = 2052
more_from_director_id = 2053
...
class People(BaseDialog):
	...
	def onAction(self, action):
		...
		if action in self.selection_actions:
			...
			if self.control_id in (2050, 2051, 2053):
				if self.control_id in (2050, 2053): db_type = 'movie'
				else: db_type = 'tvshow'
				params = {'tmdb_id': chosen_var, 'db_type': db_type, 'is_widget': 'false'}
				return dialogs.extras_menu(params)
			elif self.control_id == 2052:
				params = json.loads(chosen_var)
				chosen = dialogs.imdb_videos_choice(params['videos'], params['thumb'], media=False)
	...
	def make_more_from(self, db_type):
		try:
			...
			self.setProperty('tikiskins.person.more_from_%s.number' % db_type, '(x%02d)' % len(item_list))
