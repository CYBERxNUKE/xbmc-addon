...
xbmc.executebuiltin("XBMC.Notification(OMFG TV Guide,Please Wait!,2000)")
...
except:xbmc.executebuiltin("XBMC.Notification(OMFG TV Guide,Logo download failed,3000)")
if os.path.isfile(LocalFile):
    extractFolder = Path
    pluginsrc =  xbmc.translatePath(os.path.join(extractFolder))
    dxmnew.unzipAndMove(LocalFile,extractFolder,pluginsrc)
    xbmc.executebuiltin("XBMC.Notification(OMFG TV Guide,Logo download complete,3000)")
