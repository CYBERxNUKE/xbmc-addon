...
print '****** OMFG TV Guide Information ******'
print addonid, addonname, author, version, addonpath

ADDON     = xbmcaddon.Addon(id = 'script.tvgomfg')
MASHMODE  = (ADDON.getSetting('mashmode') == 'true')
SKIN      = ADDON.getSetting('dixie.skin')
mash_path = os.path.join(xbmc.translatePath('special://home/userdata/addon_data') , 'script.tvgomfg', 'extras', 'skins', 'Mash Up')
skin_path = os.path.join(xbmc.translatePath('special://home/userdata/addon_data') , 'script.tvgomfg', 'extras', 'skins', SKIN)
...
if not os.path.exists(skin_path):
    ...
    except:xbmc.executebuiltin("XBMC.Notification(OMFG TV Guide,Skin download failed,3000)")
...
class TVGuide(xbmcgui.WindowXML):
    ...
    def onInit(self):
        if self.initialized:
            ...
            xbmc.log("[script.tvgomfg] TVGuide.onInit(..) invoked, but we're already initialized!")
            return
