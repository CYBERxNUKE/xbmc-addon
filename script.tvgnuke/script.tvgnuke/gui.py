...
SKIN         = ADDON.getSetting('nuke.skin')
SKINSVERSION = '1'
...
try:
    if not os.path.exists(mashpath):
        ...
        except:xbmc.executebuiltin("XBMC.Notification(NUKE TV GUIDE,Skin download failed,3000)")
...
class TVGuide(xbmcgui.WindowXML):
    ...
    def onInit(self):
        if self.initialized:
            ...
            xbmc.log("[script.tvgnuke] TVGuide.onInit(..) invoked, but we're already initialized!")
            return
