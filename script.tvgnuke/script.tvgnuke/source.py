...
NUKEURL   = ADDON.getSetting('nuke.url')
NUKELOGOS = ADDON.getSetting('nuke.logo.folder')
...
if SOURCE == 'NUKE':
    logos = os.path.join(logopath, NUKELOGOS)
else:
    logos = XMLTVLOGOS
...
if NUKEURL == 'NUKE':
    ADDON.setSetting('nuke.url', 'Basic Channels')

nukecats = ADDON.getSetting('categories')
oldcats   = nukecats
fixcats   = ''
...
if oldcats == oss:
    cats_fix = nukecats.replace(oldcats, fixcats)
    # newcats
else: pass

print '****** NUKE TV GUIDE INFORMATION ******'
print '*************** LOGO PACK **************'
print logos


SETTINGS_TO_CHECK = ['source', 'xmltv.file', 'xmltv.logo.folder', 'nuke.url', 'nuke.logo.folder', 'gmtfrom']
...
class Database(object):
        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    ...
                    xbmc.log('[script.tvgnuke] Database is locked, retrying...', xbmc.LOGDEBUG)

            except sqlite3.DatabaseError:
                self.conn = None
                if self.alreadyTriedUnlinking:
                    xbmc.log('[script.tvgnuke] Database is broken and unlink() failed', xbmc.LOGDEBUG)
                    break
                else:
                    try:
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback, clearExistingProgramList):
        ...
        c = self.conn.cursor()
        try:
            xbmc.log('[script.tvgnuke] Updating caches...', xbmc.LOGDEBUG)
...
class NUKESource(Source):
    ...
    def GetDixieUrl(self):
        nukeUrl = self.nukeUrl.upper()

        if nukeUrl == 'BASIC CHANNELS':
            nukeUrl = 'http://computertechs.org/xbmc/'
            return nukeUrl
    ...
    def __init__(self, addon):
        ...
        self.nukeUrl = addon.getSetting('nuke.url')
        self.KEY += '.' + self.nukeUrl.upper()
        self.xml = None
    ...
    def getDataFromExternal(self, date, progress_callback = None): 
        ...
        if not self.xml:
            self.xml = self._downloadUrl(self.GetDixieUrl() + 'omfgxml.php')
    ...
    def getCategories(self):
        ...
        url = 'http://computertechs.org/xbmc/omfg/cats.php'
