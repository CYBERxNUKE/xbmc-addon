...
def DownloaderClass(url,dest):
    ...
    dp.create("OMFG Streams","Downloading",'[COLOR yellow]UPDATING NEW PLUGIN[/COLOR]')
...
cookie_jar = os.path.join(cookie_path, "OMFG.lwp")
...
def CATEGORIES():
    ...
    try:
        ...
        dialog = xbmcgui.Dialog()
        dialog.ok("[COLOR yellow]OMFG Streams [/COLOR]","[COLOR red]Login ERROR! [/COLOR]", "Do You Have An Account Set Up ?", "[COLOR yellow]www.OMFGstreams.com[/COLOR]")
...
def schedule(name,url,iconimage):
    ...
    if 'Al Jazeera' in For_Name:
        ...
        dialog = xbmcgui.Dialog()
        dialog.ok("OMFG Streams", '',"Sorry No Schedule Found", "")
            ...
            if desc in p:
                ...
                try:
        	        ...
        	        key = re.findall('urlkey = \'(.+?)\'',html,re.M|re.DOTALL)
                        ...
                        stream_url='%s swfUrl=http://p.jwpcdn.com/6/4/jwplayer.flash.swf timeout=15 swfVfy=true live=true pageUrl=%s'%(url.replace('" + urlkey + "',key[0]),site)
                        liz.setInfo( type="Video", infoLabels={ "Title": name} )
                        liz.setProperty("IsPlayable","true")
                        pl = xbmc.PlayList(xbmc.PLAYLIST_VIDEO)
                        pl.clear()
